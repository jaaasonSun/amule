...
final class DownloadRowValueRegressionTests: XCTestCase {
    func testPresentedDownloadRowsExposeExpectedValuesAndExcludeMalformedSharedRows() throws {
        ...
        assertVisibleRowValues(finished, expectedStatusSymbol: "checkmark.circle", expectsSources: false)
    ...
    private func assertVisibleRowValues(
        _ item: DownloadItem,
        expectedStatusSymbol: String,
        expectsSources: Bool = true,
        ...
        XCTAssertEqual(item.sourcesText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty, !expectsSources, file: file, line: line)
